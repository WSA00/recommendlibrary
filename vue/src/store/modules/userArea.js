...
export default {
    ...
    actions: {
        ...
        async updateAvatar(context, payload) {
            await api.patch(`/api/user/${payload.id}/avatar`, payload.data.avatar, { token: localStorage.getItem("token") })
        },
        // 请求接口 - 注册用户
        ...
        async fetchUserDetail({ commit }, payload) {
            const response = await api.get(`/api/user/${payload}/history`, { token: localStorage.getItem("token") })
